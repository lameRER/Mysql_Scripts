select * from rbPrintTemplate rpt where rpt.name REGEXP 'план|операц';
...
SELECT 
    ...
    c.weight AS weight, c.growth AS growth, 
...
-- 	e.*
    ...
    JOIN ActionType at1 ON a1.actionType_id = at1.id AND at1.deleted= 0 AND at1.id= 8921
    JOIN Person plv ON e.setPerson_id = plv.id and plv.deleted = 0
    ...
    WHERE e.eventType_id = 84  /*and a.id in (""" + dActions.getVar() +""")*/ /*ORDER BY nomeroperacionn, smena*/;
   ...
   explain
   select
dds.nomenclature_id, dds.id, min(dds.takeDatetime) `Начало`, max(dds.takeDatetime) `Окончание`, sum(dds.takeDose) `Доза`, rsn.mnn `МНН`
...
join ActionProperty ap on ap.action_id = a.id and ap.type_id in (SELECT apt.id from ActionPropertyType apt where apt.deleted = 0 and apt.name = 'Статус' and apt.actionType_id in (select at2.id from ActionType at2 where at2.flatCode REGEXP 'drug_theraphy|drug_theraphy_complex' and at2.deleted = 0))
...
select 
api.value, api2.value, ap.*-- , min(dds.takeDatetime) `Начало`, max(dds.takeDatetime) `Окончание`, sum(dds.takeDose) `Доза`, rsn.mnn `МНН`
from DrugDestinationSchedule dds
join Action a on a.id = dds.action_id and a.deleted = 0 /*and a.event_id = %s*/
join rbStockNomenclature rsn on rsn.id = dds.nomenclature_id and rsn.deleted = 0
join ActionProperty ap on ap.action_id = a.id and ap.type_id in (SELECT apt.id from ActionPropertyType apt where apt.deleted = 0 and apt.name REGEXP 'Статус|Код' and apt.actionType_id in (select at2.id from ActionType at2 where at2.flatCode REGEXP 'drug_theraphy|drug_theraphy_complex' and at2.deleted = 0))
left join ActionProperty_Integer api on ap.id = api.id  and (api.value = 2 or api.value = 1)
left join ActionProperty_Integer api2 on ap.id = api2.id and api2.value = dds.nomenclature_id
WHERE dds.deleted = 0 and api.id is not NULL and api2.id is not null
GROUP by a.id order by dds.id desc

   

select * from rbEpicrisisProperty rep where id = 2196;
   


select * from ActionPropertyType apt where apt.actionType_id in (select at2.id from ActionType at2 where at2.flatCode REGEXP 'drug_theraphy|drug_theraphy_complex');
   
   select * from ActionProperty ap ;
   
    
   
   select * from ActionType at2 where at2.flatCode = 'moving';
  ...
  explain
  select * from Action a, ActionType at2 where a.actionType_id = at2.id 
...
join ActionPropertyType apt on apt.id = ap.type_id and apt.deleted = 0
where ap.deleted = 0
