...
select * from (
select NOW() createDatetime, null createPerson_id, NOW() modifyDatetime, null modifyPerson_id, 
at3.code, at3.name, at3.code context, '' fileName, '' `default`, rpt.dpdAgreement, rpt.`type`, rpt.banUnkeptDate, rpt.counter_id, 0 deleted, rpt.isPatientAgreed, rpt.groupName, rpt.documentType_id, rpt.hideParam, rpt.isEditableInWeb, rpt.chkProfiles, rpt.chkPersons from ActionType at2, rbPrintTemplate rpt, 
(select * from (select "16941_1" as `code`, "ПЕРВИЧНЫЙ ОСМОТР ВРАЧА-ЭНДОКРИНОЛОГА" as `name` union select "16941_2" as `code`, "ПОВТОРНЫЙ ОСМОТР ВРАЧА-ЭНДОКРИНОЛОГА")as tmp) as at3
where at2.group_id = 20 group by at3.name) as ins
where not EXISTS (select * from rbPrintTemplate rpt where rpt.name = ins.name and rpt.code= ins.code);
