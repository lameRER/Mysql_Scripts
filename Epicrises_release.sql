...
SET @return = (SELECT GROUP_CONCAT(et.id) FROM rbEpicrisisTemplates et /*WHERE et.code = '7'*/);
...
SELECT * FROM (
SELECT @ep_ai_e:=@ep_ai_e+1, '' name, (SELECT et.name FROM rbEpicrisisTemplates et WHERE FIND_IN_SET(et.id,@return) LIMIT 1) description, 7 type, '' defaultValue
) tmp
