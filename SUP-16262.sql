...
select tie.id,
    ...
    case when tie.duplicate = 0 and tie.isImported = 1 then tie.`number` end as `number_issued_by_others_first`,
    case when tie.duplicate = 1 and tie.isImported = 1 then tie.`number` end as `number_issued_by_others_contd`,
    case when isImported = 0 then CONCAT_WS(' ', c2.lastName , c2.firstName, c2.patrName) else CONCAT_WS(' ',tie.lastName, tie.firstName, tie.patrName) end as `fullName_of_the_patient`,
    case when isImported = 0 then concat(c2.birthDate, '\n', '(',(YEAR(CURRENT_DATE) - YEAR(c2.birthDate))-(DATE_FORMAT(CURRENT_DATE, '%m%d') < DATE_FORMAT(c2.birthDate , '%m%d' )), ')') else concat(tie.birthDate, '(', tie.age, ')') end as `age`, getClientRegAddress(c2.id) as `Patient_address`,
    tie.employer as `Place_work`, tie.diagnos as `Diagnosis`, case when p2.id is null then tiep.doctor_name else CONCAT_WS(' ', p2.lastName, p2.firstName, p2.patrName) end as `doctor_issuing`,
    CONCAT_WS(' ', p3.lastName, p3.firstName, p3.patrName) as `doctor_graduated`,
    tie.begDate as `Free_work_beg`,
    tie.endDate as `Free_work_end`,
    Date(tie.endDate)-Date(tie.begDate)+1 as `Total_days_off_work`
        ...
        left join Person p3 on p3.id = tie.person_id and p3.deleted = 0
        where tie.deleted = 0 and tie.begDate >= STR_TO_DATE(@dDateBegin, '%d.%m.%Y' ) and tie.endDate <= STR_TO_DATE(@dDateEnd, '%d.%m.%Y' ) 
        UNION select tie.id, case when tie.isDuplicate=0 and tie.isImported=0 then
        ...
        case when tie.isDuplicate=0 then (YEAR(CURRENT_DATE) - YEAR(c2.birthDate))-(DATE_FORMAT(CURRENT_DATE, ' %m%d') < DATE_FORMAT(c2.birthDate , '%m%d' )) else tie.age end as `age`, getClientRegAddress(c2.id) as `Patient_address`,
