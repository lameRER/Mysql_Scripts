...
-- select e.externalId, REPLACE(tp2.`внешний идентификатор`, ' ', '') from
UPDATE 
Event e 
join Client c ON c.id = e.client_id and c.deleted = 0
join temp_person8 tp2 on tp2.фамилия = c.lastName and tp2.имя = c.firstName and tp2.отчество = c.patrName
set e.externalId = REPLACE(tp2.`внешний идентификатор`, ' ', '') 
where e.deleted = 0 and e.eventType_id = 101 and (e.externalId = '' or e.externalId != REPLACE(tp2.`внешний идентификатор`, ' ', '')) and tp2.`внешний идентификатор` is not NULL
...
select * from Event e where e.client_id in(491735,491736,491737,491738,491739);
...
select 
NOW() createDatetime, NULL createPerson_id, NOW() modifyDatetime, NULL modifyPerson_id,
0 deleted, REPLACE(tp.externalId, ' ', '') externalId, 101 eventType_id, 386271 org_id, c2.id client_id, 1 contract_id, null prevEventDate, CURDATE() setDate, Null setPerson_id, null execDate, null execPerson_id, 1 isPrimary, 1 `order`, null result_id, null nextEventDate, 0 payStatus, null typeAsset_id, '' note, null curator_id, null assistant_id, 0 pregnancyWeek, null MES_id, null HTG_id, null KSG_id, null mesSpecification_id, null relegateOrg_id, 0 totalCost, null patientModel_id, null cureType_id, null cureMethod_id, null prevEvent_id, null littleStranger_id, null referral_id, null armyReferral_id, null goal_id, null outgoingOrg_id, null outgoingRefNumber, null hmpKind_id, null hmpMethod_id, 0 eventCostPrinted, 0 exposeConfirmed, 0 ZNOFirst, 0 ZNOMorph, 0 hospParent, null clientPolicy_id, 0 isClosed, 0 dispByMobileTeam, null duration, 0 isPrivacyGuaranteeSigned, null orgStructure_id, 0 MSE 
from (select tp1.фамилия lastName, tp1.имя firstName, tp1.отчество patrName, STR_TO_DATE(REGEXP_REPLACE(if(LENGTH(tp1.`дата рож`)>9, tp1.`дата рож`, NULL), '([0-9]+).([0-9]+).([0-9]+).*', '\\1-\\2-\\3'), '%d-%m-%Y') birthDate, tp1.город city, tp1.улица street, tp1.дом house, tp1.корп corps, tp1.кварт apartment, tp1.`серия полиса` police_serial, tp1.`номер полиса` `police_number`, tp1.`страхов.компания` police_org, tp1.`внешний идентификатор` externalId, NULL doc_serial, NULL doc_number, NULL doc_date from temp_person8 tp1) as tp
left join Event e2 on e2.externalId = REPLACE(tp.externalId, ' ', '') and e2.deleted = 0 and e2.eventType_id = 101
left join Client c2 on c2.lastName = tp.lastName and c2.firstName =  tp.firstName and c2.patrName =  tp.patrName and c2.birthDate = tp.birthDate
WHERE  e2.id is NULL and tp.externalId is not null and tp.externalId != '' and 
...
tp1.город city, tp1.улица street, tp1.дом house, tp1.корп corps, tp1.кварт apartment, tp1.`серия полиса` police_serial, tp1.`номер полиса` `police_number`, tp1.`страхов.компания` police_org, tp1.`внешний идентификатор` externalId, NULL doc_serial, NULL doc_number, NULL doc_date from temp_person8 tp1) as tp
