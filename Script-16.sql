select * from rbPrintTemplate rpt where rpt.name  REGEXP 'план|опер' and rpt.id = 1286;
...
SELECT getEventDiagnosis(12701), getEventDiagnostic(12701);
...
SELECT
	aps1.value as `note`,
	GROUP_CONCAT(CONCAT(rdt.name,': ', d3.MKB) SEPARATOR '\n') as `diag`, 
    ...
    join Diagnosis d3 on d3.id = d2.diagnosis_id and d3.deleted = 0
    ...
    WHERE e.eventType_id = 84 and a.id in(37949,37987,36543)/*and a.id in (""" + dActions.getVar() +""")*/ GROUP by a.id ORDER BY nomeroperacionn, smena;
   
   
   
   
   select * from Action a where a.id = 36590;
  
  select * from ActionProperty ap where ap.id = 263574;
  
  
   
   
   select concat(c.lastName, c.firstName, c.patrName), c.weight, growth from Client c ;
