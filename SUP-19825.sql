...
#      and a.endDate = (select min(a2.endDate) from action a2 where a2.event_id= a.event_id group by a2.event_id)
...
where ai.master_id IN (86491,86401, 86400, 86478, 86491, 86492, 86300, 86301, 86546, 86446, 86377,86267, 86269, 86277) group by a.event_id having count(a.event_id) > 1;


select min(endDate)
from action where event_id in (3753130,3758282,3758317,3761676,3765005,3769520,3769610,3772792,3781126,3782069,3783789,3784265,3790106,3790847,3790854,3791105,3791107,3793745,3796828,3796845,3796885) group by event_id;
